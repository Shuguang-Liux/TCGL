...
import com.cosmo.elemes.common.exception.BusinessException;
import com.cosmo.elemes.common.utils.StringUtils;
...
//                    throw new ADIException("不存在的数据源："+datasource,500);
    ...
    public boolean createDataSource(String key, String driveClass, String url, String username, String password, String databaseType) {
        try {
            ...
            hikariDataSource.setPoolName(key);
            hikariDataSource.setUsername(username);
            hikariDataSource.setPassword(password);
            hikariDataSource.setConnectionTestQuery(DbDriver.getValidationQuery(databaseType));
...
//            String validationQuery = DbDriver.getValidationQuery(databaseType);
//
...
//            druidDataSource.init();
            this.dynamicTargetDataSources.put(key, hikariDataSource);
    ...
    public boolean testDatasource(DbInfo dbInfo) {
        String driveClass = DbDriver.getDriverClass(dbInfo.type());
        return testDatasource(null, driveClass, dbInfo.url(), dbInfo.username(), dbInfo.password());
    }
    ...
    public void createDataSourceWithCheck(DbInfo dbInfo) throws Exception {
        String datasourceId = dbInfo.name();
        ...
        if (dynamicTargetDataSources2.containsKey(datasourceId)) {
            ...
            HikariDataSource druidDataSource = (HikariDataSource) dynamicTargetDataSources2.get(datasourceId);
...
//                }
    ...
    private void createDataSource(DbInfo dbInfo) throws Exception {
        String datasourceId = dbInfo.name();
        ...
        if(testDatasource(datasourceId, driveClass, url, username, password)) {
            boolean result = this.createDataSource(datasourceId, driveClass, url, username, password, dbInfo.type());
            if(!result) {
